...
foreach ($controlPlaneFolder in (Get-ChildItem -Path './CloudFormation' -Recurse -Filter '*ControlPlane-Parameters' -Directory)) {
    foreach ($file in (Get-ChildItem -Path $controlPlaneFolder.FullName -Filter '*.json' -Recurse)) {
        ...
        $fileContent.Parameters.ArtifactS3Bucket = $artifactBucket
